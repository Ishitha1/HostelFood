...
class ViewFeedbackFragment : Fragment() {
   ...
   /* private fun exportToExcel() {
        ...
        val file = File(requireContext().getExternalFilesDir(null), "Weekly_Feedback.xlsx")
            ...
            .addOnSuccessListener { documents ->
                ...
                for (doc in documents) {
                    ...
                    row.createCell(0).setCellValue(
                        doc.getTimestamp("timestamp")?.toDate()?.let {
                            SimpleDateFormat("dd-MM-yyyy HH:mm").format(it)
                        } ?: ""
                    )

                    row.createCell(1).setCellValue(doc.getString("day") ?: "")
                    row.createCell(2).setCellValue(doc.getString("rollNumber") ?: "")
                    row.createCell(3).setCellValue(doc.getString("mealType") ?: "")
